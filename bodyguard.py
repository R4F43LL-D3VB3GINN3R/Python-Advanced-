...
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
class Bodyguard(pygame.sprite.Sprite):
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
    ...
    def __init__(self, pos):  # Este é o método construtor da classe Enemies, que é chamado automaticamente quando uma nova instância (objeto) da classe é criada. 
        ...
        self.image = self.animations['idle'][self.frame_index]    # self.image recebe self.animations que é um dicionário declarado dentro do método import_character_assets().
        ...
        self.scale_factor = 3                                     # Tamanho do sprite. 
...
# Todas as animacões são armazenadas dentro de um dicionário.
    ...
    def import_character_assets(self):  # O método import_character_assets é responsável por carregar as animações do inimigo a partir de arquivos de imagem.
        ...
        character_path = './graphics/enemies/bodyguard/'  # Define o caminho para a pasta que contém os recursos gráficos do personagem
        self.animations = {'idle':[], 'attack1':[], 'attack2':[], 'defend':[], 'hurt':[]}  # Um dicionário que será preenchido com listas de imagens para diferentes animações do inimigo.
    ...
    def animate(self):  # O método animate atualiza o sprite do inimigo para a próxima imagem na sequência de animação.
        ...
        if self.status == 'defend' and self.frame_index >= len(animation): # Se o índice atual ultrapassar ou for igual ao comprimento da lista de imagens e o status for defend...
            self.frame_index = 2                                           # Seta o frame índex como 2.
        elif self.frame_index >= len(animation): # Se o índice atual (self.frame_index) ultrapassou ou é igual ao comprimento da lista de imagens da animação. 
